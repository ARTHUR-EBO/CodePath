...
module.exports = {
  async listar(req, res) {
    try {
      ...
      // Para cada exercício, adiciona as alternativas
      const exerciciosComAlternativas = await Promise.all(
        exercicios.map(async (ex) => {
          const alternativas = await Alternativa.getByExercicio(ex.id);
          return { ...ex, alternativas };
        })
      );

      res.json(exerciciosComAlternativas);
    } catch (error) {
      console.error(error);
